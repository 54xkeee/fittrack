...
| PERF-01 | P1 | `ExerciseListModel` 每个动作额外查询肌肉和媒体；58 项一次重载约产生 117 次查询，且当前有两份动作模型 | 搜索、筛选和首次加载存在不必要开销 |
| PERF-02 | P1 | `Main.qml` 的 `StackLayout` 启动时常驻实例化五个主页面 | 首屏加载了当前暂时不可见的复杂页面 |
| PERF-03 | P1 | 三份动作 JSON 每次启动都重新事务导入 | 启动阶段执行重复工作 |
| PERF-04 | 已关闭 | CMake 只打包 `resources/images/exercises/shareable/`；58 张引用图全部进入 APK，MuscleDB、旧目录和 Inter 字体均未进入 | 包内探针与种子测试已验证 |
| PERF-05 | P1 | 历史控制器启动时加载全部历史，页面用 `ScrollView + Repeater` 实例化全部卡片 | 长期自用数据增长后会持续变慢 |
| PERF-06 | P1 | 活动训练每次改组、排序等操作后全量重载 session，并逐动作、逐组查询 | 训练越长，单次操作代价越高 |
...
| UI-01 | P1 | 当前大量使用 Unicode 字符作为图标、嵌套菜单、连续描边卡片和高饱和荧光按钮 | 信息层级弱，触感和视觉观感偏旧 |
| TEST-01 | P0 | 现有排序测试只直接调用 C++ 方法，没有模拟真实触摸拖动；启动测试没有覆盖 active 冲突 | “14 项测试通过”不能证明用户反馈的流程可用 |
| TEST-02 | P1 | 当前截图测试只保存 PNG，不与批准基准图做差异比较；QML 主流程集中在一个大测试函数 | 视觉退化不会让测试失败，前一步失败会跳过大量后续场景 |
| A11Y-01 | P1 | 应用将系统字体缩放上限截断为 1.5 | 无法覆盖 Android 200% 大字体使用场景 |
...
- 生产应用已移除 Inter，改为系统字体；桌面测试、QML lint 目标、14/14 自动化测试、Android Lint 和 APK 包内媒体检查通过。真实 TalkBack、ColorOS 大字体和后台行为仍需连接一加 Ace 5 Pro 验收。
...
- 页面延迟加载、种子版本化导入、搜索防抖。
- 三个大页面拆成有清晰职责的组件。
- QML Profiler 和 Android Perfetto 对比报告。
...
退出门禁：三档屏幕、三档字体截图通过；不再出现字符图标、截断主操作或难以理解的隐藏菜单。
...
- `tst_exercisepreviewflow`：计划、选择器、训练三个入口和两图切换。
...
| 训练准备草稿 | 不存在 | 不存在 | 不存在 | 不存在 | 未实现 |
| 计划动作参数保存 | 已存在 | 部分存在 | 部分截图 | 未验收 | 部分完成 |
| 恢复默认 | 不存在 | 不存在 | 不存在 | 不存在 | 未实现 |
| 计划间歇进入训练 | 当前有缺陷 | 当前测试未区分数值 | 未验证 | 未验证 | 未完成 |
| 动作库详情 | 已存在 | 已覆盖单图、署名、许可证和专业字段 | 已有截图 | 未验收 | 桌面完成 |
| 计划/训练动作详情 | 不存在 | 不存在 | 不存在 | 不存在 | 未实现 |
| 添加动作前预览 | 不存在 | 不存在 | 不存在 | 不存在 | 未实现 |
| C++ 排序 | 已存在 | 基础测试通过 | 不代表 UI 可用 | 未验收 | 部分完成 |
| 触摸拖动排序 | 当前实现有缺陷 | 不存在 | 用户反馈不可用 | 未验收 | 未完成 |
| 性能 | 未建立指标 | 不存在 | 用户反馈不流畅 | 未测 | 未验收 |
| 视觉系统 | 有主题 token | 有部分截图 | 用户认为观感陈旧 | 未验收 | 需重构 |
| 200% 大字体 | 当前被截断 | 未覆盖 | 未验证 | 未验证 | 未完成 |
| 视觉基准比较 | 不存在 | 仅生成截图 | 不能自动发现退化 | 未验证 | 未实现 |
| 字体文档一致性 | 文档与资源/代码存在冲突 | 未覆盖 | 未验证 | 未验证 | 需审计 |
...
- 三档屏幕、1.5 倍字体、TalkBack 和 Android 返回键通过。
