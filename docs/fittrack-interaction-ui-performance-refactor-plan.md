...
| MAINT-01 | 部分关闭 | `TrainingPage.qml`、`PlanPage.qml`、`ExerciseLibraryPage.qml` 当前分别为 1663、1319、1641 行；训练页已拆出 `CurrentSetInputPanel`、`TrainingRestTimer`、`EquipmentChoiceDialog` | 三个高内聚区域已拆分，但训练页仍负责总体流程与大量弹层；文件大本身不等于性能问题，剩余工作以降低回归风险为目标 |
...
| TEST-02 | 部分关闭 | Windows 固定环境已对 5 个静态关键状态建立像素容差基准；QML 主流程仍集中在一个大测试函数 | 视觉退化已能自动失败；前一步失败跳过后续场景的维护风险仍在 |
...
2026-07-14 的 15/15 与聚焦 `qmlnavigation` 结果保留为当时阶段证据。2026-07-22 的当前分支已通过 Qt 6.11.1 构建和 14 项非视觉测试；`qmlnavigation` 在当前 Windows headless 环境进入测试输出前超时。下文 R2/R5 的记录保留历史证据，最新结论以本节、验收矩阵和 `fittrack-platform-ui-audit.md` 为准。
...
- R5 桌面全量构建、QML lint、15/15 自动化测试、360×800/2.0 倍准备页与 360×640/1.5 倍训练页截图、Android arm64 Debug 构建均于 2026-07-14 通过。原生多层菜单和真机 TalkBack/ColorOS 字体仍未关闭，因此这里只记为桌面证据完成，不记为最终验收。
...
- R6 视觉门禁从 75 张输出中只冻结 5 个无动态时间的关键状态。独立运行实测中 3 张逐像素一致，恢复提示坏点比例 0.058%、动作详情 0.002%；门禁要求尺寸一致、单通道差值大于 8 的坏点不超过 0.1%、全图 RGB MAE 不超过 0.25，并在失败时输出差异图。普通准备页因动画差异达 4.44% 而明确未纳入，避免伪稳定测试。
- R6 Debug 侧载包由 `package-side-load.ps1` 重复生成：硬性校验包名、单一 `arm64-v8a`、Debug 证书、v2 签名和 APK 哈希，输出仅 1 个 APK，并附许可正文、实际 Qt SBOM、NDK NOTICE、媒体署名、源码/重新链接说明和逐文件 SHA-256。迁移到 Qt 6.11.1、`com.xuke.fittrack` 和 API 29/36 后必须重新生成并校验，旧产物不能作为当前发布证据。
...
| 视觉基准比较 | Windows 固定 Qt 6.11.1/字体/离屏软件渲染环境 | 5 个静态关键状态执行尺寸、坏点比例与 RGB MAE 比较，动作图片显式等待 Ready | 15/15 与随后聚焦 `qmlnavigation` 均通过；失败时仍保留 actual、expected 与 diff | Android 自动化已在 API 36 模拟器回归，真机未验证 | 真机可用后补视觉、TalkBack、通知和厂商后台策略 |
| Debug 侧载分发 | `package-side-load.ps1` | 包名、ABI、Debug/v2 签名、APK 复制哈希、许可/SBOM 数量及逐文件哈希自校验 | 脚本能力已具备，升级后产物待重新生成 | 未安装 | 重新校验后再进入真机安装 |
