...
- R4 已完成固定查询与懒加载重构：动作目录 117→3 条 SQL、三日计划详情 12→6、12 动作/48 组训练恢复 83→12、历史详情固定 5；未变化动作种子 981→1，未变化计划种子 1 条。
- 首页分析启动固定 3 条 SQL，首次进入分析详情追加 5 条；动作模型、计划/历史/分析/管理页按需加载。有氧首页只取 1 条，页面按 50 条分页，趋势固定最近 30 次。
- SQLite 升级到 v8，补齐排序、筛选与级联删除索引，并增加同版本启动快路径。桌面 15 项测试、QML lint、真实 360×800/2.0 字体与触摸导航、Android arm64 Debug 构建均通过。
...
| 系统大字体（至 2.0 倍） | 已统一映射到设计令牌 | 训练主操作区、训练准备、14 个操作弹层和 4 个确认框已覆盖 | 360×640/1.5 倍、360×800/2.0 倍截图通过 | 待 ColorOS 真机 | 桌面自动化完成 |
