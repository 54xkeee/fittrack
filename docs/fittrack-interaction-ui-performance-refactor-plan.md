...
状态：R1–R3 桌面门禁满足；R4 性能代码完成但训练快照查询门槛未满足；R5 桌面自动化与截图完成；R6 待实施
...
| PERF-06 | 部分关闭 | 12 动作/48 组训练恢复由 83 条降至固定 12 条 SQL | 仍高于第 9 节冻结的 ≤5 条门槛，不能写成性能验收完成 |
...
- SQLite 升级到 v8，补齐排序、筛选与级联删除索引，并增加同版本启动快路径。桌面 15 项测试、QML lint、真实 360×800/2.0 字体与触摸导航、Android arm64 Debug 构建均通过。训练恢复仍为 12 条 SQL，高于第 9 节冻结的 5 条门槛，因此 R4 性能最终验收保持开放。
...
| 性能 | 固定批量 SQL、按需页面、分页、种子摘要和 SQLite v8 快路径 | 动作 117→3、计划 12→6、训练 83→12、历史详情 5、动作种子 981→1、计划种子 1 | 桌面 SQL 门禁通过 | 冷/热启动、帧时间、PSS 未测 | 桌面优化已完成；训练 12 条仍高于 ≤5 门槛，真机未验收 |
