...
- R6 Debug 侧载包改为 `package-side-load.ps1` 重复生成：硬性校验包名、单一 `arm64-v8a`、Debug 证书、v2 签名和 APK 哈希，输出仅 1 个 APK，并附 26 份许可正文、3 份实际 Qt SBOM、NDK NOTICE、媒体署名、源码/重新链接说明和逐文件 SHA-256。脚本正向自校验通过，长期 Release 密钥仍待用户保管方案。
...
| 视觉基准比较 | Windows 固定 Qt 6.9.1/字体/离屏软件渲染环境 | 5 个静态关键状态执行尺寸、坏点比例与 RGB MAE 比较，动作图片显式等待 Ready | actual、expected 与 diff 可定位失败 | Android 未验证 | 桌面门禁完成，待真机视觉 |
| Debug 侧载分发 | `package-side-load.ps1` | 包名、ABI、Debug/v2 签名、APK 复制哈希、许可/SBOM 数量及逐文件哈希自校验 | 本地生成目录和 ZIP 通过 | 未安装 | 离线分发材料完成，待真机安装 |
