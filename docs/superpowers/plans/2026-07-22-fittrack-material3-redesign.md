...
- Round 3（进行中）：已建立 `AppScaffold`、Top App Bar、Navigation Bar、Bottom Sheet、Snackbar、OverlayHost 与 TaskFeedbackHost；主底部导航和训练准备顶栏已迁移到组件层，动作详情、动作选择、参数与排序已迁移为 Bottom Sheet，保留原 `objectName`、焦点和无障碍页签语义。Windows Qt 6.11.1 构建通过。
