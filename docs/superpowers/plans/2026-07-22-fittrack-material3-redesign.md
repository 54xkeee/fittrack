...
- Round 5（进行中）：休息控制已从训练滚动内容移到固定 Bottom Action Bar，滚动区域会为其预留空间；计时设置继续使用 Bottom Sheet；训练完成后会直接进入独立训练总结，关闭后回到实际训练来源；完成组与完成训练已接入 Android 原生分级触感，桌面端保持无操作；训练/动作备注的编辑表面已抽取为独立 `TrainingNoteSheet`。Windows Qt 6.11.1 构建通过。
