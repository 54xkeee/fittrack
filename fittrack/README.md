...
FitTrack 是一个面向 Android 的个人健身训练记录与分析应用。当前桌面开发版已完成训练记录闭环、谭成义三分化与自由训练、个人计划管理、训练历史、容量与 e1RM 分析、有氧记录、健身房/器械区分、备份恢复，以及 32 个动作的离线资料库。
...
- Graphite & Lime 设计系统已经提取为主题单例与通用组件，完成底部导航、首页、训练、计划和历史页面的移动端重构；动作库、趋势、有氧与管理页面仍需进入同一组件体系。
...
QML 导航测试会把五个主页面、训练进行中、历史详情、有氧和管理页面分别按 360×800、420×920、480×1056 写入 `C:\FitTrackDev\fittrack\build\visual`。默认使用 `offscreen` 与软件渲染，可直接在无交互窗口的测试环境运行；如需检查本机图形后端可使用：
