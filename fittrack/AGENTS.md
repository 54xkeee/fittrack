...
- 当前数据库为 SQLite v6；v5 为动作增加难度、发力要点、常见错误和动作集合字段，v6 增加单一 active 训练约束。个人训练日单段有氧通过 `plan_cardio` 保存，开始训练时复制到 `workout_cardio_target`，训练后只预填实际有氧记录。
...
Android 当前基线为包名 `com.fittrack.app`、min API 28、target/compile API 35、仅 `arm64-v8a`。Debug APK 必须通过 Android Lint、`aapt` 清单检查、`apksigner` 校验和包内媒体白名单检查；自动化还要覆盖 360×640、1.5 倍字体、TalkBack 语义、48dp 触控区和真实 `QTouchEvent`。一加 Ace 5 Pro 的后台计时、通知拒绝、SAF、返回键、ColorOS 电池策略、TalkBack、大字体和数字键盘仍需连接真机验收。Release 长期签名、AAB 和商店材料不属于当前验收范围。构建与排障见 [`../docs/fittrack-android-build.md`](../docs/fittrack-android-build.md)。
